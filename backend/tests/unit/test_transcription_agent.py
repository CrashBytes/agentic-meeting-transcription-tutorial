...
class TestTranscriptionAgent:
    ...
    def test_agent_initialization_auto_device(self):
        ...
        with patch(
            "app.agents.transcription_agent.torch.cuda.is_available", return_value=False
        ):
    ...
    async def test_transcribe_chunk_success(
        self, agent, sample_audio_chunk, sample_whisper_result
    ):
    ...
    async def test_transcribe_file_success(
        self, agent, test_audio_file, sample_whisper_result
    ):
    ...
    def test_calculate_confidence_no_logprob(self, agent):
        ...
        result = {"segments": [{}, {"avg_logprob": -0.5}]}  # No avg_logprob field
    ...
    @pytest.mark.asyncio
    async def test_transcribe_chunk_temperature_parameter(
        self, agent, sample_audio_chunk
    ):
    ...
    async def test_transcribe_different_languages(self, agent, sample_audio_chunk):
        ...
        agent.language = "es"
        agent.model.transcribe = Mock(
            return_value={"text": "Hola mundo", "language": "es", "segments": []}
        )
    ...
    def test_cleanup_on_deletion(self, agent):
        ...
        with patch(
            "app.agents.transcription_agent.torch.cuda.empty_cache"
        ) as mock_cache:
            agent.device = "cuda"
            del agent
    ...
    @pytest.mark.asyncio
    async def test_transcribe_chunk_confidence_threshold(
        self, agent, sample_audio_chunk
    ):
        ...
        agent.model.transcribe = Mock(
            return_value={
                "text": "Uncertain text",
                "language": "en",
                "segments": [{"avg_logprob": -5.0}],  # Very low confidence
            }
        )
