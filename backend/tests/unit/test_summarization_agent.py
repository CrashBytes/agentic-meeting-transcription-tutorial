...
class TestSummarizationAgent:
    ...
    async def test_summarize_brief(self, agent, sample_transcript):
        ...
        result = await agent.summarize(sample_transcript, detail_level="brief")
    ...
    async def test_summarize_medium(self, agent, sample_transcript):
        ...
        result = await agent.summarize(sample_transcript, detail_level="medium")
    ...
    async def test_summarize_detailed(self, agent, sample_transcript):
        ...
        result = await agent.summarize(sample_transcript, detail_level="detailed")
    ...
    async def test_summarize_all_levels(self, agent, sample_transcript):
        ...
        result = await agent.summarize(sample_transcript, detail_level="all")
    ...
    async def test_summarize_with_context(self, agent, sample_transcript):
        ...
        agent.llm.ainvoke = AsyncMock(
            return_value=Mock(content="Summary incorporating historical context")
        )

        result = await agent.summarize(
            sample_transcript, context=context, detail_level="medium"
        )
    ...
    async def test_generate_brief_summary_directly(self, agent):
        ...
        agent.llm.ainvoke = AsyncMock(return_value=Mock(content="Test brief summary"))

        result = await agent._generate_brief_summary("Test transcript", "Test context")
    ...
    async def test_generate_medium_summary_directly(self, agent):
        ...
        agent.llm.ainvoke = AsyncMock(return_value=Mock(content="Test medium summary"))

        result = await agent._generate_medium_summary("Test transcript", "Test context")
    ...
    async def test_generate_detailed_summary_directly(self, agent):
        ...
        agent.llm.ainvoke = AsyncMock(
            return_value=Mock(content="Test detailed summary")
        )

        result = await agent._generate_detailed_summary(
            "Test transcript", "Test context"
        )
    ...
    async def test_summarize_long_transcript(self, agent):
        ...
        agent.llm.ainvoke = AsyncMock(
            return_value=Mock(content="Summary of long transcript")
        )

        result = await agent.summarize(long_transcript, detail_level="brief")
